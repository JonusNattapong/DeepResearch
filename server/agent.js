...
class SandboxAgent {
  ...
  async run(url, onLog, onChunk, onDraftUpdate) {
    ...
    try {
      ...
      this.messages.push({ role: "user", content: `Target: ${url}. Begin swarm investigation. You can delegate tasks to 'Friend Agents' if needed.` });
      ...
      while (!done && turnCount < MAX_TURNS) {
        ...
        if (turnCount === MAX_TURNS) {
           ...
           this.messages.push({ role: "user", content: buildBlueprintSynthesisPrompt(draftSnapshot, url, mode.toUpperCase()) });
        }
